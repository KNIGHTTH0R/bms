...
$this->title ='Owner Detail';
$this->params['breadcrumbs'][] = ['label' => 'Units', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;

$detail_owner = json_decode($model->owner->detail_person); //Company
      ...
      <div class="box box-widget widget-user-2">
       <!-- Add the bg color to the header using any of the bg-* classes -->
        <div class="widget-user-header bg-yellow">
          ...
          <h3 class="widget-user-username"><?= $detail_owner->salutation ? $detail_owner->salutation :''?> <?= $model->owner->name ?></h3>
          <h5 class="widget-user-desc">TAX ID: <?= $model->owner->tax_reg ?></h5>
        </div>
        <div class="box-footer no-padding">
          <ul class="nav nav-stacked">
            <li><a href="#">UNIT <span class="pull-right"><?= $model->unit_floor . ' - ' . $model->unit_num ?></span></a></li>
            <li><a href="#">TYPE ID <span class="pull-right"><?= $detail_owner->id_type ? $detail_owner->id_type : '-' ?></span></a></li>
