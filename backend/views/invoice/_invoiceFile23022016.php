				...
				<td valign="top" style="border:solid 1px #000; width:80%; border-top:none; line-height:30px;">
					...
					<table cellpadding="3" style="font-size:11px">
						...
						if($model->tariffRecur=='MONTH'){
							...
							$newdate = strtotime('-'.$hari.' day',$model->charge_date);
							$newdate = date('d F Y', $newdate); 
						}else if($model->tariffRecur=='3MONTH'){

							$jumbul1 = $model->charge_month;
							$jumbul2 = $model->charge_month-1;
							$jumbul3 = $model->charge_month-2;
							$jumtah = date('Y');
							...
							// $newdate = strtotime('-'.$hari.' day',$model->charge_date);
							// $newdate = date('d F Y', $newdate); 
							$monthch= $model->charge_month-2;
							$yearsub= substr($model->charge_year,-2);
							$newdate = $monthch.'/'.$model->tariffPdf->recur_date.'/'.$yearsub;
						...
						?>
							...
							<td style="font-size:10px; font-weight:bold;" colspan="7">Periode Pemakaian : <?php echo $newdate.' s/d '.date('d F Y', $model->charge_date-1); ?></td>
