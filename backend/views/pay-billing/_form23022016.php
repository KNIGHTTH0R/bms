            ...
            <p>Periode Pemakaian : <?= date('d F Y', $newdate).' s/d '. date('d F Y', $model2->unitChargeValue->charge_date-1) ?></p>
              <?php 
                    ...
                    foreach ($model as $index => $models) {
                		...
                		echo '<div class="row pay-detail'.$noDetail.'"><div class="col-md-3">'.$form->field($models, "[$index]total_charge")->textInput(['value' => number_format($totalCharge,0,',','.'), 'readonly' => true])->label('Value Charge').'</div>';
                        echo '<div class="col-md-3">'.$form->field($models, "[$index]total_pay")->textInput(['id'=>'number'.$noDetail, 'value'=>0, 'placeholder'=>'0'])->label('Value Pay').'</div>';
                        echo '<div class="col-md-3">'.$form->field($models, "[$index]id")->textInput(['value'=>$models->id])->label('Value Pay').$form->field($models, "[$index]type")->hiddenInput(['value'=>$models->type])->label(false).'</div>';
