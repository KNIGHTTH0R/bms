...
class GenerateController extends Controller
{
    ...
    public function actionCreate()
    {
        $model = new \yii\base\DynamicModel([
            'month', 'year', 'interval', 'month3', 'month6', 'yearly'
        ]);
        $model->addRule(['month','year','month3','month6','yearly'], 'required');
                ...
                if($queryMeter==null) {
                    ...
                    return $this->redirect(['unit-charge-value/index']);
                        ...
                        foreach ($queryCharge as $queryCharge) {
                                ...
                                $cekCount = UnitChargeValue::find()->where(['charge_month'=>$bulan6, 'type'=>$queryCharge->type, 'unit_code'=>$queryCharge->unit->code])->count();
                                ...
                                if ($queryCharge->type == 'SINKINGFUND') {
                                    ...
                                    $dataCoa = Coa::find()->where(['name' => 'A/R SINKING FUND'])->one();
                                    $dataCoaPiutang = Coa::find()->where(['name' => 'INCOME SINKING FUND'])->one();
                                ...
                                $dueDateFormula2 = mktime(0,0,0,$model->month3,$queryCharge->tariff->recur_date+20,date('Y'));
                                ...
                                $modelChargeValue2->value_penalty = $penaltyValue2;
                                $modelChargeValue2->detail = $rumus2;
                                $modelChargeValue2->charge_date =strtotime($queryCharge->tariff->recur_date.'.'.$model->month3.'.'.date('Y'));
                                $modelChargeValue2->due_date = $dueDateFormula2;
                                $modelChargeValue2->charge_year = $model->year;
                        ...
                        foreach ($queryCharge as $queryCharge) {
                                ...
                                // $dueDateFormula2 = strtotime($queryCharge->recur_date.' '.date('F').' '.date('Y'))+strtotime($ddFor2.' '.date('F').' '.date('Y'));
                                $dueDateFormula2 = mktime(0,0,0,$model->month6,$queryCharge->tariff->recur_date+20,date('Y'));
                                $modelChargeValue2->type = $queryCharge->tariff->type;
                                $modelChargeValue2->unit_charge_id = $queryCharge->id;
                                $modelChargeValue2->value_charge = $tariffValue2;
                                $modelChargeValue2->value_tax = ($taxValue2/100)*$tariffValue2;
                                ...
                                $modelChargeValue2->value_penalty = $penaltyValue2;
                                $modelChargeValue2->detail = $rumus2;
                                $modelChargeValue2->charge_date =strtotime($queryCharge->tariff->recur_date.'.'.$model->month6.'.'.date('Y'));
                                $modelChargeValue2->due_date = $dueDateFormula2;
                                $modelChargeValue2->charge_year = $model->year;
                        ...
                        foreach ($queryCharge as $queryCharge) {
                                ...
                                // $dueDateFormula2 = strtotime($queryCharge->recur_date.' '.date('F').' '.date('Y'))+strtotime($ddFor2.' '.date('F').' '.date('Y'));
                                $dueDateFormula2 = mktime(0,0,0,$model->month12,$queryCharge->tariff->recur_date+20,date('Y'));
                                $modelChargeValue2->type = $queryCharge->tariff->type;
                                $modelChargeValue2->unit_charge_id = $queryCharge->id;
                                $modelChargeValue2->value_charge = $tariffValue2;
                                $modelChargeValue2->value_tax = ($taxValue2/100)*$tariffValue2;
                                ...
                                $modelChargeValue2->value_penalty = $penaltyValue2;
                                $modelChargeValue2->detail = $rumus2;
                                $modelChargeValue2->charge_date =strtotime($queryCharge->tariff->recur_date.'.'.$model->month12.'.'.date('Y'));
                                $modelChargeValue2->due_date = $dueDateFormula2;
                                $modelChargeValue2->charge_year = $model->year;
