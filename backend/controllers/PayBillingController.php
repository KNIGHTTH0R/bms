...
class PayBillingController extends Controller
{
    ...
    */
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($modelTambah->load(Yii::$app->request->post())) {
                ...
                if($dataDeposit->explan==TRUE){
                     ...
                     $newDeposit = $model->total_pay  - $modelTambah->value_charge;
                     ...
                     if($model->total_pay < $modelTambah->value_charge){
                        ...
                        // $modelDeposit->explan = 'Deposit Pembayaran inv '.$model->inv_number;
                            ...
                            $modelPayBill = PayBilling::find()->where(['inv_number'=>$invnumber])->one();
                            $modelPayBill->total_pay = $newDeposit;
                            $modelPayBill->update();
                        ...
                        $JurnalDepositChild2->amount = $model->total_pay;
                    ...
                    if($model->total_pay < $modelTambah->value_charge){
                        ...
                        // $modelDeposit->explan = 'Deposit Pembayaran inv '.$model->inv_number;
                            ...
                            $modelPayBill = PayBilling::find()->where(['inv_number'=>$invnumber])->one();
                            $modelPayBill->total_pay = $model->total_pay;
                            $modelPayBill->update();
    ...
    public function actionPrint($invnumber){

        $model = PayBilling::find()->where(['inv_number'=>$invnumber])->one();
        if($model->status_pay=='NOK'){
           $modelPrint = PayBilling::find()->where(['inv_number'=>$invnumber])->all();
           $modelPrintHead = PayBilling::find()->where(['inv_number'=>$invnumber])->one();
           return $this->renderPartial('printdeposit',[
            'modelPrint'=>$modelPrint,
            'modelPrintHead' => $modelPrintHead,
            ]); 
        }else{
            $modelPrint = PayBilling::find()->where(['inv_number'=>$invnumber])->all();
            $modelPrintHead = PayBilling::find()->where(['inv_number'=>$invnumber])->one();
                return $this->renderPartial('printreceipt',[
            'modelPrint'=>$modelPrint,
            'modelPrintHead' => $modelPrintHead,
            ]); 
        }
